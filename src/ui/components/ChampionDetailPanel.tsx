...
export function ChampionDetailPanel({ row, role, tiers, tierContext, onSetTier, onOpenStats, onClose }: { row: ChampionRoleStat; role: string; tiers: Record<string, string>; tierContext: TierContext; onSetTier: (championId: string, tier: string) => void; onOpenStats: (championId: string, role: string) => void; onClose: () => void }) {
  const t = useT();
  ...
  return <aside className="champion-detail-panel" aria-label={`${row.championName} ${t("championDetail.detailsAria")}`}>
    ...
    <section className="champion-detail-performance">
      <div><span className="detail-label">{t("championDetail.adjustedWinRate")}</span><strong>{formatPercent(row.adjustedWinRate)}</strong><small>{t("championDetail.adjustedWinRateHint", { games: row.games })}</small></div>
      <button type="button" className="detail-stats-link" onClick={() => onOpenStats(row.championId, row.role)} aria-label={t("championDetail.openStatsAria", { champion: row.championName })}><IconChartBar size={15} />{t("championDetail.openStats")}</button>
    </section>
